...
def rand_val(index):
    if index == 0:
        ...
        return ["best","random"][random.randint(0, 1)]
    elif index == 2:
...
def DecisionTreeParametersFeatureFitness(y, df, numberOfAttributes, individual):
    ...
    estimator = DecisionTreeClassifier(criterion=individual[0], splitter=individual[1], max_depth=individual[2], min_samples_leaf=individual[3])
    resultSum = 0
